...
def update_item(user_id, category_id, item_name, priority_number, due_date, new_item_name, new_priority_number, new_due_date):
    cur.execute('''UPDATE TodoItems SET item_name = ?, priority_number = ?, due_date = ? WHERE user_id = ? AND category_id = ? AND item_name = ? AND priority_number = ? AND due_date = ?''',
                (new_item_name, new_priority_number, new_due_date, user_id, category_id, item_name, priority_number, due_date))
    conn.commit()
...
def delete_item(user_id, category_id, item_name, priority_number, due_date):
    cur.execute('''DELETE FROM TodoItems WHERE user_id = ? AND category_id = ? AND item_name = ? AND priority_number = ? AND due_date = ?''',
                (user_id, category_id, item_name, priority_number, due_date))
    conn.commit()
...
def get_category_id(category_name):
    cur.execute(
        '''SELECT id FROM Category WHERE category_name = ?''', (category_name,))
